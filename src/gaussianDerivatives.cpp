...
gaussianDerivatives::gaussianDerivatives(){
	...
	kernelSize = 5;
	kernelType = CV_32F;
	gaussianKernel = getGaussianKernel(kernelSize, 1, kernelType);
...
gaussianDerivatives::gaussianDerivatives(int ksize, double sigma, int ktype){
	kernelSize = ksize;
	kernelType = ktype;
	gaussianKernel = getGaussianKernel(kernelSize, sigma, kernelType);
	sigmaX = sigma > 0 ? sigma : ((kernelSize-1)*0.5 - 1)*0.3 + 0.8;
	scale2X = -0.5/(sigmaX*sigmaX);
}
Mat gaussianDerivatives::getDerivativeKernel(int order){
	Mat kernel = gaussianKernel;
